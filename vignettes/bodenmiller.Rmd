...
cell.colors <- setNames(c('#9CA5D5', '#0015C5', '#5B6CB4', '#BFC5E8', '#C79ED0', '#850094', '#A567B1',
                          '#DBBCE2', '#D3C6A1','#5E4500', '#BBDEB1', '#8A1923', '#B35E62', '#CEA191'),
                        c('cd14-hladr-', 'cd14-hladrhigh', 'cd14-hladrmid', 'cd14-surf-', 'cd14+hladr-',
...
ggplot(untreatedFuncFrame %>% filter(Cells=='cd4+') %>% group_by(Treatment,Cells,channel) %>% do(do.fan(.$value)),
  ...
  guides(fill=F)+
  facet_wrap(~Cells*Treatment,ncol=1,scale='free_x')+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  scale_fill_fan()
```
